    ...
    <div className={`ticket ${used ? "used" : ""}`}>
      <div className="ticket-stub">
        ...
        >
          ...
          {used ? (
            <span className="qr-used-overlay" aria-hidden="true">
              <span className="qr-used-stamp">
                <Icon name="checkCircle" size={15} />
                <span>{locale === "km" ? "បានប្រើរួច" : "USED"}</span>
              </span>
            </span>
          ) : (
        ...
        <span className={`tiny with-icon ${used ? "ticket-stub-used" : ""}`}>
          <Icon name={used ? "checkCircle" : "ticket"} size={12} />
      ...
      <div className="ticket-info">
        ...
        {used && (
          <span
            className="badge badge-used"
            style={{ alignSelf: "flex-start" }}
          >
            <Icon name="checkCircle" size={12} />
            {t("alreadyUsed")}
            {ticket.checked_in_at ? ` · ${dateTime(ticket.checked_in_at)}` : ""}
          </span>
        )}
        ...
        <span className="qr-token">
          {ticket.qr_token ?? ticket.booking_ref}
        </span>
      </div>
    </div>
  );
