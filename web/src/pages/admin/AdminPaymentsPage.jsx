...
export default function AdminPaymentsPage() {
  ...
  function exportCsv() {
    const columns = [
      {
        header: "Provider",
        value: (p) => PROVIDER_LABEL[p.provider] ?? p.provider,
      },
  ...
  async function checkWithProvider(payment) {
    ...
    try {
      ...
      if (!result.checked) {
        ...
        toast(
          km ? "បានទទួលប្រាក់រួចរាល់" : "Settled — the provider had the money",
          "success",
        );
      } else {
        toast(
  ...
  const pagedPayments = usePaging(
    payments,
    `${provider}|${status}|${eventId}|${stuckOnly}`,
  );
  ...
  const totals = payments.reduce(
    (acc, p) => ({
      ...
      pending:
        acc.pending +
        (["PENDING", "CREATED"].includes(p.status) ? p.amount_usd_cents : 0),
    }),
          ...
          <p>
            {browsingEvents ? (
              ...
              <>
                ...
                {totals.count} {km ? "ប្រតិបត្តិការ" : "attempts"} ·{" "}
                {usd(totals.success)} {km ? "ជោគជ័យ" : "settled"} ·{" "}
                {usd(totals.pending)} {km ? "រង់ចាំ" : "in flight"}
              </>
            )}
          </p>
        </div>
        ...
        <div className="panel">
          ...
          {loadingHealth ? (
            ...
            <div className="panel-body">
              <Empty
                icon="card"
                title={km ? "គ្មានការទូទាត់ទេ" : "No payments yet"}
              >
                {km
