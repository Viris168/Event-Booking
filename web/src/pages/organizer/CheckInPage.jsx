...
const HEADLINE = {
  ...
  TICKET_NOT_IN_PARTY: {
    en: "Selection out of date — rescan",
    km: "ជម្រើសផុតកំណត់ — សូមស្កេនម្តងទៀត",
  },
};
  ...
  const scan = useCallback(
    async (raw) => {
      ...
      try {
         ...
         */
        const preview = mapGroupPreview(
          await previewGroup(value, Number(eventId)),
        );
        setCode("");
        ...
        if (preview.total === 1) {
          ...
          const result = mapScanResult(
            await scanTicket(value, Number(eventId)),
          );
          setLast(result);
          if (result.ticket) {
            setRecent((prev) =>
              [{ ...result, at: new Date().toISOString() }, ...prev].slice(
                0,
                8,
              ),
            );
  ...
  const admitParty = useCallback(
    async (howMany) => {
      ...
      try {
        ...
        setError(
          km ? "មិនអាចភ្ជាប់ទៅម៉ាស៊ីនមេ" : "Could not reach the server.",
        );
      } finally {
        setBusy(false);
      }
      ...
      <div className="split">
            ...
            <div className="panel-body stack-sm">
              ...
              >
                {eventsLoading ? (
                  ...
                  <SkeletonRegion
                    label={km ? "កំពុងផ្ទុក…" : "Loading events…"}
                  >
              ...
              >
                <Field
                  label={t("manualEntry")}
                  hint="EBT1.42.ARaBt9WwSFCg1I2WcHYqLA.pfBnW1S…"
                >
                  <div className="row">
                    <input
        ...
        <div className="panel">
          <div className="panel-head">
            ...
            {recent.length > 0 && (
              <span className="scan-count-badge">{recent.length}</span>
            )}
          </div>
          <div className="panel-body">
            {recent.length ? (
              <ul className="timeline">
                {recent.map((r, i) => {
                  const dot = TONE[r.outcome] ?? "bad";
                  return (
                    <li
                      key={`${r.ticket?.ticket_id}-${i}`}
                      className={`dot-${dot}`}
                    >
                      <div>
                        <b>
...
function Scanner({ active, onDecode, onError }) {
    ...
    import("html5-qrcode")
      .then(({ Html5Qrcode, Html5QrcodeSupportedFormats }) => {
        ...
        return instance.start(
          ...
          (text) => {
            const now = Date.now();
            if (seen.current.value === text && now - seen.current.at < 3000)
              return;
  ...
  if (!active) {
    return (
      <div className="scanner-idle">
        <span className="scanner-idle-icon">
          <Icon name="qr" size={22} />
        </span>
        <span>
          {locale === "km"
            ? "កាមេរ៉ាបិទ — ចុចប៊ូតុងខាងលើ"
            : "Camera off — tap button above"}
        </span>
      </div>
    );
  }
  ...
  return (
    <div className="scan-corners">
      <div id="gate-scanner" ref={holder} className="scan-live" />
    </div>
  );
}
