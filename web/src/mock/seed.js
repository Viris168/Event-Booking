...
export function buildSeed() {
  ...
  function addEvent(cfg) {
    ...
    if (cfg.classes) {
      ...
      const seatsOfVenue = venueSeats.filter(
        (s) => s.venue_id === event.venue_id,
      );
    ...
    classes: [
      {
        ...
        section_label: "Zone A",
      },
      {
        name_en: "Zone B",
        name_km: "តំបន់ ខ",
        price_usd_cents: 2500,
        section_label: "Zone B",
      },
    ],
    soldRatio: 0.34,
  });
  ...
  addEvent({
    ...
    description_en:
      "Sunset-to-sunrise beach party. Line-up still being confirmed.",
    description_km:
      "ពិធីជប់លៀងឆ្នេរពីថ្ងៃលិចដល់ថ្ងៃរះ។ បញ្ជីសិល្បករកំពុងបញ្ជាក់។",
    status: "DRAFT",
    starts_at: days(75),
  ...
  addEvent({
    ...
    description_en:
      "Taken down pending a content review after audience complaints.",
    description_km:
      "ត្រូវបានដកចេញ ដើម្បីរង់ចាំការត្រួតពិនិត្យខ្លឹមសារ បន្ទាប់ពីមានពាក្យបណ្តឹងពីទស្សនិកជន។",
    status: "TAKEN_DOWN",
    starts_at: days(30),
  ...
  addEvent({
    ...
    description_en:
      "Weekly sundown DJ set on the sand. Bring nothing, buy a drink, stay for the stars.",
    description_km:
  ...
  addEvent({
    ...
    description_en:
      "Eight comedians, two languages, one microphone. Khmer and English sets alternate.",
    description_km:
  ...
  addEvent({
    ...
    description_en:
      "Programme still being confirmed with the visiting directors.",
  ...
  addEvent({
    ...
    description_en:
      "Taken down after the organizer could not verify the advertised speakers.",
    description_km:
      "ត្រូវបានដកចេញ បន្ទាប់ពីអ្នករៀបចំមិនអាចផ្ទៀងផ្ទាត់វាគ្មិនដែលបានផ្សាយ។",
    status: "TAKEN_DOWN",
    starts_at: days(45),
  ...
  function seedBooking(cfg) {
    ...
    const holdStatus =
      cfg.state === "CONFIRMED" ||
      cfg.state === "REFUND_REQUESTED" ||
      cfg.state === "REFUNDED"
        ? "CONSUMED"
        : cfg.state === "PENDING_PAYMENT" ||
            cfg.state === "AWAITING_CONFIRMATION" ||
            cfg.state === "PAYMENT_FAILED"
          ? "ACTIVE"
          : cfg.state === "EXPIRED"
            ? "EXPIRED"
            : "RELEASED";
    ...
    for (const spec of cfg.seats || []) {
      const cls = seatClasses.find(
        (c) => c.event_id === event.id && c.name_en === spec.className,
      );
      const pool = eventSeats.filter(
        (s) =>
          s.event_id === event.id &&
          s.seat_class_id === cls.id &&
          s.status === "AVAILABLE",
      );
    ...
    for (const spec of cfg.zones || []) {
      const zone = eventZones.find(
        (z) => z.event_id === event.id && z.name_en === spec.zoneName,
      );
      if (!zone) continue;
      if (holdStatus === "CONSUMED") {
      ...
      for (const it of items) {
        for (let seq = 1; seq <= it.qty; seq++) {
          tickets.push({
            ...
            checked_in_at:
              cfg.checkedIn &&
              (cfg.allCheckedIn || (seq === 1 && it === items[0]))
                ? cfg.checkedIn
                : null,
            checked_in_by:
              cfg.checkedIn &&
    ...
    for (const p of cfg.paymentAttempts || []) {
      payments.push({
        ...
        expires_at:
          p.expires_at ||
          new Date(new Date(cfg.createdAt).getTime() + 900000).toISOString(),
        created_at: p.created_at || cfg.createdAt,
        resolved_at: ["SUCCESS", "FAILED", "CANCELLED", "EXPIRED"].includes(
          p.status,
        )
    ...
    paymentAttempts: [
      {
        ...
        status: "SUCCESS",
        provider_ref: "BKG-99120031",
        currency_charged: "KHR",
      },
    ],
    history: [
      { from: null, to: "PENDING_PAYMENT", by: 1, at: days(-6) },
      {
        ...
        to: "CONFIRMED",
        by: null,
        note: "Bakong webhook BKG-99120031",
        at: days(-6),
      },
    ],
    checkedIn: days(-1),
  ...
  seedBooking({
    ...
    history: [
      { from: null, to: "PENDING_PAYMENT", by: 1, at: hours(-2) },
      {
        ...
        by: null,
        note: "PayWay returned, no webhook yet",
        at: hours(-2),
      },
    ],
  });
    ...
    paymentAttempts: [
      {
        provider: "ABA_PAYWAY",
        status: "FAILED",
        provider_ref: "ABA-TXN-71880",
      },
      {
        provider: "BAKONG_KHQR",
        status: "FAILED",
        provider_ref: "BKG-98770012",
      },
    ],
    history: [
  ...
  const paid = (provider, ref) => [
    { provider, status: "SUCCESS", provider_ref: ref },
  ];
